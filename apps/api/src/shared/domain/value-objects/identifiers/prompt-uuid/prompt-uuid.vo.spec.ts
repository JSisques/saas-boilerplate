...
describe('PromptUuidValueObject', () => {
  const validUuid = '123e4567-e89b-12d3-a456-426614174000';

  it('should be an instance of UuidValueObject', () => {
    const uuid = new PromptUuidValueObject(validUuid);
  ...
  it('should generate random UUID when no value provided', () => {
    const uuid = new PromptUuidValueObject();
  ...
  it('should throw InvalidUuidException for invalid UUID', () => {
    expect(() => new PromptUuidValueObject('invalid')).toThrow(
      InvalidUuidException,
    );
  });

  it('should support equals method', () => {
    const uuid1 = new PromptUuidValueObject(validUuid);
    const uuid2 = new PromptUuidValueObject(validUuid);
