...
export class SubscriptionPlanViewModelFactory
  implements
    ...
    >
{
  private readonly logger = new Logger(SubscriptionPlanViewModelFactory.name);
  ...
  public create(
    ...
    this.logger.log(
      `Creating subscription plan view model from DTO: ${JSON.stringify(data)}`,
    );
  ...
  fromPrimitives(
    ...
    this.logger.log(
      `Creating subscription plan view model from primitives: ${subscriptionPlanPrimitives}`,
    );
  ...
  fromAggregate(
    ...
    this.logger.log(
      `Creating subscription plan view model from aggregate: ${subscriptionPlanAggregate}`,
    );
    ...
    return new SubscriptionPlanViewModel({
      ...
      description: subscriptionPlanAggregate.description.value,
      ...
      trialPeriodDays: subscriptionPlanAggregate.trialPeriodDays.value,
      isActive: subscriptionPlanAggregate.isActive.value,
      features: subscriptionPlanAggregate.features.value,
      limits: subscriptionPlanAggregate.limits.value,
      stripePriceId: subscriptionPlanAggregate.stripePriceId.value,
      createdAt: now,
      updatedAt: now,
    });
  }
